...
def visualize_interactive(data, strategy_name, initial_capital):
    ...
    # Adjust the layout to increase the chart height
    fig1.update_layout(
        title=f'{strategy_name} Strategy - Buy/Sell Signals',
        xaxis_title='Date',
        yaxis_title='Price',
        height=800, width=1200  # Increased height for the signals chart
...
def main():
    ...
    with st.sidebar:
        ...
        # Extract the start and end date from the dataset
        data_min_date = data.index.min().date()
        data_max_date = data.index.max().date()

        # Set the default start and end date to match the dataset's available range
        start_date = st.text_input("2. Enter Start Date (YYYY-MM-DD)", str(data_min_date))
        end_date = st.text_input("3. Enter End Date (YYYY-MM-DD)", str(data_max_date))
